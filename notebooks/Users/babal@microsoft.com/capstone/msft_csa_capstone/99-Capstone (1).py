...
def writeToBronze(sourceDataframe, bronzePath, streamName):
  (sourceDataframe
    ...
    #.withColumn("eventTime", (col("eventParams.client_event_time").cast("timestamp")))
    #.filter(col("eventDate") =!= null)
...
df = spark.read.format("delta").load(lookupPath)
display(df)
...
dbutils.fs.rm(outputPathSilver + "/_checkpoint", recurse=True)
...
df = spark.read.format("delta").load(outputPathSilver)
display(df)
...
df1 = df.groupBy(weekofyear("eventDate")).agg(F.countDistinct("device_id").alias("WAU"), F.min(weekofyear("eventDate")).alias("week"), F.min("eventDate").alias("eventDate")).select("WAU","week","eventDate")
...
# MAGIC %sql
# MAGIC 
# MAGIC CREATE TABLE mobile_events_delta_silver (WAU INT, week INT)

# COMMAND ----------

# MAGIC %sql 
# MAGIC DROP table mobile_events_delta_gold;
# MAGIC DROP table mobile_events_delta_silver;
# MAGIC DROP table mobile_events_delta_silver1;
...
# MAGIC CREATE TABLE mobile_events_delta_silver (WAU INT, week INT)
# MAGIC   USING DELTA
# MAGIC   --LOCATION '/mnt/training/gaming_data/mobile_streaming_events/gaming/gold'

# COMMAND ----------

dbutils.fs.rm(outputPathGold + "/_checkpoint", recurse=True)

# COMMAND ----------

dbutils.fs.rm("dbfs:/user/babal@microsoft.com/capstone/gaming/gold", recurse=True)

# COMMAND ----------

dbutils.fs.ls("dbfs:/user/babal@microsoft.com/capstone/gaming/gold")
...
df1.select("WAU","week").write.format("delta").save(outputPathGold)

# COMMAND ----------

spark.sql("CREATE TABLE mobile_events_delta_gold USING DELTA LOCATION '" + outputPathGold + "'")
...
# MAGIC -- TODO
# MAGIC 
# MAGIC select week, WAU from mobile_events_delta_gold
